...
#nms算法：cv共识：坐标系建的是左上！！
# 1.首先求出所有检测出来的矩形面积；对数据按照分数降序排列好
# 2.遍历矩形数据，取出第一条数据，这个是分数最高的数据；然后把这个分数最高的索引保存到数组中
# 3.取出所有矩形与该分数最高数据的交集区域；
# 4.计算交集对应的长宽若为负数，证明无交集，赋值为0
# 5.把交集面积除以两个矩形的面积极为IOU的值
# 6. 取出所有IOU大于阈值的数据，这数据可以认为与分值最高的数据为一个检测体
# 7. 取出剩余的数据的坐标然后对数据坐标加一，因为我们已经把分值最高的取出来了，所以永远少一个
# 8.然后把坐标取index里面取出对应的数据，即代表index中的这些数据是没有被第一轮移除
# 9.重复执行直到index集合中没有数据，
...
while index.size > 0:
    ...
    print(keep)
    print('x1',x1[i])
    print('x1[index[1:]]',x1[index[1:]])
    print('y1',y1[i])
    print('y1[index[1:]]',y1[index[1:]])
    print('x2',x2[i])
    print('x2[index[1:]]',x2[index[1:]])
    print('y2',y2[i])
    print('y2[index[1:]]',y2[index[1:]])
    # 计算交集的左上角和右下角
    ...
    print("areas[index[1:]]",areas[index[1:]])
    ...
    print("np.where",np.where(ious <= thresh))
